...
1. Поднял проект
2. Добавил в корень проекта файл homeBudget.json с бюджетом для sitespeed.io
3. Проверил, что бюджет не соблюдается:
   **[2020-03-08 19:30:28] ERROR: Failing budget JavaScript Transfer Size for http://localhost:3000 with value 1.0 MB max limit 449.2 KB**
   (использовал команду: `sitespeed.io --budget.configPath homeBudget.json -n 1 http://localhost:3000)`

4. Добавил к проекту webpack-bundle-analyzer
5. Сделал анализ - убедился, что moment.js входит в сборку vendor
6. Закомментил proCharts.js и сделал еще раз анализ в webpack-bundle-analyzer - _так не грузится ни vendor, ни moment_

по поводу Cleanup

- так как после комментирования proCharts.js, moment.js вообще не фигурирует в отчете webpack-bundle-analyzer, я сделал вывод, что moment.js используется только для Chart.js.
  То есть я не совсем понимаю, куда подключать moment, если proCharts закомменченны?
  Нашел, как импортнуть Chart.js без moment - но там все равно далеко до соблюдения бюджета.

**_тут возникает сложность в понимании задания: мы оставляем Chart.js закомменченным или это только идея для оптимизации?_**  
**_если оставляем - то мы просто выпиливаем нужный функционал сайта? А если нет - то загрузка получается около 700 Кб, а бюджет - 449,2 Кб_**

Надо заметить, что я совсем не соображаю в React-е. И в целом по фронту на весьма базовом уровне работал.

И еще боль - даже с закомменченным proCharts.js тест не проходит (чуть чуть не хватает):
**[2020-03-08 20:00:18] ERROR: Failing budget JavaScript Transfer Size for http://localhost:3000 with value 451.1 KB max limit 449.2 KB**
Я позволил себе немного увеличить бюджет, чтобы тест проходил.

7. Поставил ngrok, запустил его с локальным проектом.
8. Добавил конфиг CI для github.
9. Запушил ветку.
